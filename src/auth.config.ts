...
export const authConfig: NextAuthConfig = {
    ...
    callbacks: {
        ...
        async jwt({ token, user, trigger, session }) {
            ...
            // Si el usuario acaba de iniciar sesión, asignar valores iniciales
            if (user) {
                token.data = user || {}; // Asegurar que siempre sea un objeto
                token.accessToken = user.token as string;
                token.exp = now + 7200; // Expira en 2 horas
            }

            // Si se ejecuta update(), actualizar los datos sin perder el accessToken
            if (trigger === "update") {
