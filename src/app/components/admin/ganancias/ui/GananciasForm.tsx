"use client";

import { useForm } from "react-hook-form";
import { enqueueSnackbar } from "notistack";
import { ButtonLoading } from "@/app/components/ui/buttons/ButtonLoading";
import { ButtonAnimated } from "@/app/components/ui/buttons/ButtonAnimated";
import { useEffect, useState } from "react";
import { readGanancias } from "@/actions/orders/readGanancias.action";
import { MonthlyChart } from "../MonthlyChart";
import { Ganancias } from "@/app/interfaces/ganancias";

export interface GananciasDTO {
    month?: string;
    year?: number;
    typo?: "servicio" | "pack";
}

const months = [
    { value: "0", label: "Enero" },
    { value: "1", label: "Febrero" },
    { value: "2", label: "Marzo" },
    { value: "3", label: "Abril" },
    { value: "4", label: "Mayo" },
    { value: "5", label: "Junio" },
    { value: "6", label: "Julio" },
    { value: "7", label: "Agosto" },
    { value: "8", label: "Septiembre" },
    { value: "9", label: "Octubre" },
    { value: "10", label: "Noviembre" },
    { value: "11", label: "Diciembre" },
];

export const GananciasForm = () => {
    const currentDate = new Date();
    const currentMonth = currentDate.getMonth().toString();
    const currentYear = currentDate.getFullYear();
    const years = Array.from({ length: currentYear - 2000 + 1 }, (_, i) => (2000 + i).toString());

    const { register, handleSubmit, formState: { isSubmitting } } = useForm<GananciasDTO>({
        defaultValues: {
            year: currentYear,
            month: currentMonth,
        }
    });
    const [filters, setFilters] = useState<GananciasDTO>({ month: undefined, year: undefined, typo: undefined });

    const [ganancias, setGanancias] = useState<Ganancias | null>(null);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        const fetchGanancias = async () => {
            setLoading(true);
            const { ok, ganancias } = await readGanancias(filters);
            if (ok) setGanancias(ganancias!);
            setLoading(false);
        };

        fetchGanancias();
    }, [filters]); // Se ejecuta cuando cambian los filtros



    const onSubmit = (data: GananciasDTO) => {
        setFilters(data);
        enqueueSnackbar("Filtros actualizados", { variant: "success" });
    };

    return (
        <section className="w-full">
            <form onSubmit={handleSubmit(onSubmit)} className="py-5 w-full" autoComplete="off">
                <div className="flex gap-4">
                    {/* Select de mes */}
                    <select {...register("month")} className="border rounded p-2">
                        <option value="">Seleccionar mes</option>
                        {months.map((m) => (
                            <option key={m.value} value={m.value}>{m.label}</option>
                        ))}
                    </select>

                    {/* Input de año */}
                    <select {...register("year")} className="border rounded p-2">
                        {years.map((year) => (
                            <option key={year} value={year}>{year}</option>
                        ))}
                    </select>

                    {/* Select de tipo */}
                    <select {...register("typo")} className="border rounded p-2">
                        <option value="">Todos los tipos</option>
                        <option value="servicio">Servicio</option>
                        <option value="pack">Pack</option>
                    </select>

                    {isSubmitting ? <ButtonLoading label="Buscando..." /> : <ButtonAnimated label="Buscar" />}
                </div>

            </form>

            {loading ? (
                <p>Cargando datos...</p>
            ) : (
                ganancias && (
                    <>
                        <h3>Ganancias 
                            <span className="mx-1">
                                {
                                    filters.typo ? `${filters.typo}` : 'totales'
                                }
                            </span>
                            
                             del mes: ${ganancias.totalGanancias} </h3>
                        <MonthlyChart month={ganancias.month} />
                    </>
                )
            )}

        </section>
    );
};
